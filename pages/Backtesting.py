...
st.markdown("<h3 style='text-align: left;'>Resumen de Backtesting</h3>", unsafe_allow_html=True)

resumen_html = resumen.style.set_table_styles(
    [{
        'selector': 'th',
        'props': [('background-color', 'black'), ('color', 'white'), ('text-align', 'center')]
    }]
).set_properties(**{
    'text-align': 'center',
    'width': '120px'
...
st.markdown(f"<div style='text-align: left'>{resumen_html}</div>", unsafe_allow_html=True)
...
# Formatear tendencia y condiciones
...
cond1 = 'True' if last_row['VIX_C_y'] <= 25 else 'False'
cond2 = 'True' if (last_row['VIX_C_y'] < last_row['VIX_WMA_21_y'] and last_row['VIX_WMA_21_y'] < last_row['VIX_WMA_21_2dy']) else 'False'
...
tabla_prediccion = pd.DataFrame([{
    ...
    'Tendencia': f"<span style='color:{tendencia_color}'>{tendencia}</span>",
    'Last VIX': round(last_row['VIX_C_y'], 2),
    'Last VIX_WMA_21': round(last_row['VIX_WMA_21_y'], 2),
    'VIX < 25': f"<span style='color:{'green' if cond1 == 'True' else 'red'}'>{cond1}</span>",
    'VIX < Last 1-2 WMA21': f"<span style='color:{'green' if cond2 == 'True' else 'red'}'>{cond2}</span>"
}])

# Mostrar predicción con estilo
st.markdown("<h3 style='text-align: left;'>Predicción del Próximo Día de Negociación</h3>", unsafe_allow_html=True)

tabla_html = tabla_prediccion.to_html(escape=False, index=False)
...
<div style="text-align: left">{tabla_html}</div>
"""
